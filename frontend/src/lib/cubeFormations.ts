 ...
 */
export type FormationPattern = 'dock' | 'grid' | 'circle' | 'arc' | 'constellation' | 'scattered';
...
export function calculateArcFormation(
  ...
  const angleStep = arcAngle / (cubeCount - 1);
...
export function calculateConstellationFormation(
  ...
  for (let i = 0; i < cubeCount; i++) {
    ...
    const radius = spread * Math.sqrt(i / cubeCount);
...
/**
 * Calculate cube positions for a scattered formation (truly random)
 */
