...
export function BackendArchitecturePanel({}: BackendArchitecturePanelProps) {
  ...
  const [activeView, setActiveView] = useState<'services' | 'routes' | 'capabilities' | 'permissions'>('services');
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {activeView === 'routes' && <RoutesView routes={data.routes} plugins={data.plugins} />}
        {activeView === 'capabilities' && <CapabilitiesView capabilities={data.capabilities} />}
        {activeView === 'permissions' && <PermissionsView plugins={data.plugins} metrics={data.metrics} />}
      </div>
    </div>
  );
}
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
          Shows how large "God Object" services have been split into focused sub-services with
          single responsibilities. Composition layers maintain backward compatibility.
        </p>
      </div>
...
function RoutesView({ routes, plugins }: RoutesViewProps) {
  ...
  const findPluginForRoute = (path: string) => {
    ...
    return plugins.find(p => path.includes(p.id.replace('_', '-')) || path.includes(p.id));
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
          Available capability APIs that PluginContext provides to routes. These form the clean
          architecture layer between routes and domain logic.
        </p>
      </div>
...
function PermissionsView({ plugins, metrics }: PermissionsViewProps) {
  ...
  const matrix: Array<{
    ...
    plugins: plugins.filter(p => p.permissions.includes(perm)).map(p => p.name),
  }));
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
          Shows which permissions are declared by which plugins. PluginContext checks these
          permissions before allowing capability API operations.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <MetricCard
          label="Total Uses"
          value={Object.values(metrics.permission_usage).reduce((a, b) => a + b, 0)}
...
function MetricCard({
  ...
  return (
    <div className="p-3 bg-neutral-50 dark:bg-neutral-800 rounded-md border border-neutral-200 dark:border-neutral-700">
      ...
      {sublabel && (
        <div className="text-xs text-neutral-500 dark:text-neutral-400">{sublabel}</div>
      )}
    </div>
  );
}
