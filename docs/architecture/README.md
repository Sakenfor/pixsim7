...
### Migration to httpx (2025-12-12)
The `pixverse-py` library migrated from `requests` to `httpx` (async HTTP):
- ✅ Updated all `client.list_videos()` calls to await coroutines
- ✅ Updated all `client.list_images()` calls to await coroutines
- ✅ Removed `asyncio.to_thread()` wrappers
- ✅ Verified `client.create()` and `client.get_video()` already async

### PROVIDER_PRICING Cleanup (2025-12-12)
- ✅ Removed legacy USD pricing stub
- ✅ Unified on credit-based pricing via `pixverse_pricing` helpers
- ✅ Simplified cost estimation endpoints
