...
- **Capability Registry**: Manages features, routes, actions, and reactive state. Enables modular feature registration and permission-based access control.
- **Plugin Ecosystem**: Extensible plugin system supporting multiple plugin kinds (helpers, interactions, node types, gallery tools, world tools, UI plugins, generation providers).
- **Module System**: Feature modules that register capabilities and initialize subsystems on app startup.
- **Scene/Quest Editor**: Graph-based visual editor for creating interactive scenes, quests, and narrative flows.
- **Game Frontends**: Multiple game/simulation UIs including Game2D (2D world exploration), NpcBrainLab (NPC behavior testing), and Simulation Playground (system testing).
- **Generation System**: AI-powered content generation with pluggable UI providers and prompt engineering tools.
...
**Purpose:** Centralized registry for app features, routes, actions, and state. Modules register capabilities during app initialization.

**Code Locations:**
- `frontend/src/lib/capabilities/` - Core capability registry and hooks
- `frontend/src/modules/` - Feature modules (scene-builder, automation, gallery, etc.)
- `frontend/src/lib/capabilities/registerCoreFeatures.ts` - Core feature registration

**Key Hooks:**
- `useFeatures()` - Get all enabled features
- `useFeatureRoutes(featureId)` - Get routes for a feature
- `useActions()` - Get all registered actions
- `useStateValue<T>(id)` - Reactive state access
- `useCapabilityPermission()` - Permission checking

**Documentation:**
- [APP_CAPABILITY_REGISTRY.md](./APP_CAPABILITY_REGISTRY.md) - Capability system architecture
- [CAPABILITY_HOOKS.md](./CAPABILITY_HOOKS.md) - Hook API reference and usage patterns
- [CAPABILITY_PLUGIN_INTEGRATION.md](./CAPABILITY_PLUGIN_INTEGRATION.md) - Plugin-capability integration

**How It Works:**
- Modules call `registerCompleteFeature()` to register a feature with its routes, actions, and state
- Capability store (`useCapabilityStore`) maintains the registry
- React hooks provide reactive access to capabilities throughout the app
- Permission system controls feature/action access based on user roles
