...
interface OverlayConfiguration {
  ...
  allowOverflow?: boolean; // If false, clamp positions to container bounds
}
...
- **Pointer vs. touch**: Visibility triggers like `hover` should degrade to `always` or `focus` on touch-only devices; codify the fallback.
