...
> **Status:** ARCHIVED
> **Date:** 2025-11-16
> **Reason:** This spec describes a tick-based node engine for world simulation in `pixsim7_game_service`. Game logic was **consolidated into the main backend** without tick-based simulation. Current scene graphs are **narrative/video flow only**, not world simulation.

---

## For Current Scene Implementation, See:

**Current scene architecture (implemented):**
- `docs/SYSTEM_OVERVIEW.md` – Overview of current scene system
- `docs/NODE_EDITOR_DEVELOPMENT.md` – Current scene graph editor
- `docs/PHASE4_CANONICAL_SCENE_SCHEMA.md` – Current `@pixsim7/types.Scene` schema
- `packages/types/src/index.ts` – Actual Scene/Node/Edge types

**Current design:**
- Scene graphs are for **narrative/video flow only** (not world simulation)
- No tick-based execution engine
- Scenes executed on-demand via `ScenePlayer` component
- World state managed via `GameSession.flags` and `GameSession.relationships` (not graph-driven)

---

## This Document Describes (Not Implemented):

A **future node-based world simulation engine** that could unify scene graphs and NPC behavior graphs. Not currently implemented.
