 ...
 */
function toFlowNodes(arcGraph: any): Node[] {
  if (!arcGraph?.nodes) return [];

  return arcGraph.nodes.map((node: ArcGraphNode) => ({
    id: node.id,
    type: 'scene', // Reuse SceneNode component
 ...
 */
function toFlowEdges(arcGraph: any): Edge[] {
...
export function ArcGraphPanel() {
  ...
  const nodeTypes = useMemo<NodeTypes>(
    () => ({
      scene: SceneNode, // Reuse SceneNode component
    }),
    []
  );
  ...
  const onNodeDoubleClick = useCallback(
    (_event: React.MouseEvent, node: Node) => {
      const arcNode = node.data as ArcGraphNode;

      if (arcNode.type === 'arc' && arcNode.sceneId) {
        // Navigate to scene graph editor
      ...
      <div className="flex-1 flex relative">
        ...
        {showPalette && (
          <NodePalette
            ...
            scope="arc" // Filter to arc scope nodes
          />
        )}
