...
export function App() {
  ...
  // Bootstrap: load services + connect WebSocket
  useEffect(() => {
    loadServices()
    connect()
  }, [])
  ...
  return (
    <div className="h-screen flex flex-col bg-surface text-gray-100">
        ...
        <div className="w-72 shrink-0 border-r border-border flex flex-col">
          ...
          <div className="flex items-center justify-between px-3 py-2.5 border-b border-border">
            ...
            <div className="flex gap-1">
              <button
                onClick={startAll}
                className="px-2 py-0.5 text-[10px] font-medium rounded bg-green-700 hover:bg-green-600 text-white transition-colors"
                title="Start All"
              >
                Start All
              </button>
              <button
                onClick={stopAll}
                className="px-2 py-0.5 text-[10px] font-medium rounded bg-red-700 hover:bg-red-600 text-white transition-colors"
                title="Stop All"
              >
                Stop All
              </button>
      ...
      {/* Error banner */}
      {error && (
        <div className="px-3 py-1.5 bg-red-900/40 text-red-300 text-xs border-t border-red-800/50">
          {error}
        </div>
      )}
