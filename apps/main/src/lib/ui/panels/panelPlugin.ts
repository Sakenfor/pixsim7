...
export class PanelPluginManager {
  ...
  async loadPlugin(plugin: PanelPlugin): Promise<void> {
    ...
    if (plugin.requires && !this.checkDependencies(plugin)) {
      const missing = plugin.requires.filter((dep) => !this.loadedPlugins.has(dep));
      throw new Error(
    ...
    // Check conflicts
    if (plugin.conflicts) {
      const conflicts = plugin.conflicts.filter((conflict) => this.loadedPlugins.has(conflict));
    ...
    console.log(`Plugin "${plugin.id}" loaded successfully with ${plugin.panels.length} panels`);
  }
