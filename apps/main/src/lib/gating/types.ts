 ...
 *
 * Based on feedback from GPT-5 and aligned with stat package architecture.
 *
...
export interface GatingPlugin {
   ...
   *   { statDefinitionId: 'relationships', axes: { affinity: 60, chemistry: 50 } },
   *   'romantic',
   *   { minimumLevel: 'light_flirt' }
   * );
   * if (!result.allowed) {
   ...
   * @example
   * ```ts
   * const reqs = plugin.getGateRequirements('mature', config);
   * console.log(reqs.axisThresholds); // { affinity: 60, chemistry: 50 }
   * console.log(reqs.requiredLevelIds); // ['intimate']
   * ```
   */
  getGateRequirements(
    gateType: string,
