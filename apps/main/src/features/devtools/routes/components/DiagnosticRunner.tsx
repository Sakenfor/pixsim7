...
export function DiagnosticRunner({
  ...
  const { currentPhase, latestPerSource, transitions, summary, terminal } = useMemo(() => {
    ...
    for (const ev of events) {
      if (ev.type === 'phase' && typeof ev.phase === 'string') {
        ...
        const tValue = toFiniteNumber(ev.value) ?? toFiniteNumber(ev.t_rel);
        if (tValue !== undefined) trans.set(ev.key, tValue);
  ...
  const elapsed = useMemo(() => {
    for (let i = events.length - 1; i >= 0; i -= 1) {
      const t = toFiniteNumber(events[i]?.t_rel);
      if (t !== undefined) return t;
    }
    return 0;
  }, [events]);

  const displayStatus = useMemo<DiagnosticRunSummary['status'] | undefined>(() => {
    if (!run) return undefined;
    if (terminal?.status) return terminal.status;
    if (run.status !== 'running' && connection === 'open') {
      // Stream is still open and terminal not observed yet; avoid premature completed badge.
      return 'running';
    }
    return run.status;
  }, [run, terminal, connection]);
      ...
      <div className="flex items-center gap-3 rounded border border-cyan-500/40 bg-cyan-500/5 px-3 py-2">
        ...
        {run && (
          <>
            ...
            <span className={`rounded border px-2 py-0.5 text-xs font-semibold uppercase ${statusPill(displayStatus)}`}>
              {displayStatus}
            </span>
          </>
        )}
            ...
            return (
              <div key={p.key} className="flex items-center gap-2">
                ...
                >
                  ...
                  {!isActive && isPast && <span className="mr-1">OK</span>}
                  {p.label}
                </span>
              </div>
            );
