...
export function BlockFitDevPage() {
  // Input fields
  ...
  const handleComputeFit = async () => {
    ...
    try {
      const data = await pixsimClient.post<FitScoreResponse>('/dev/block-fit/score', {
        block_id: blockId,
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      await pixsimClient.post('/dev/block-fit/rate', {
        block_id: blockId,
        asset_id: parseInt(assetId, 10),
  ...
  useEffect(() => {
    ...
    if (promptVersionIdParam) {
      ...
      pixsimClient.get(`/dev/prompt-library/versions/${promptVersionIdParam}`)
        .then((data: any) => {
  ...
  useEffect(() => {
    ...
    pixsimClient.get(`/assets/${assetId}`)
      .then((data: any) => {
        setAssetInfo({
  ...
  const fetchExistingRatings = async () => {
    ...
    try {
      ...
      const data = await pixsimClient.get<any>(`/dev/block-fit/list?${params}`);
