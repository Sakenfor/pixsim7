import type { SceneBeat, ScenePlan } from '@pixsim7/shared.types';
  ...
  return blocks.map((block, index) => {
    ...
    const beat: SceneBeat = {
      ...
      actorTargets: [
        {
          ...
          anchorId: locationId != null ? `location:${locationId}:primary` : 'world:origin',
          pathIntent: index === 0 ? 'arrive' : 'interact',
        },
      ...
      camera: category === 'camera'
        ...
        : undefined,
      meta: {
        sourceBlockId: blockId(block, index),
      },
    };
    cursor += durationSec;
    return beat;
...
function buildScenePlanPreview(args: {
  ...
  const { request, built, selection, fallbackWorldTime } = args;
  ...
  const beats = buildBeats(selection, request, derived, locationId);
