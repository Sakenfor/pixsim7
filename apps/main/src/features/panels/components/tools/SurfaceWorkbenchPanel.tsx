...
function formatCategory(category?: string): string {
  ...
  return (
    CATEGORY_LABELS[category] ?? category.replace(/^\w/, (c) => c.toUpperCase())
  );
}
      ...
      messages.push({
        ...
        content: (
          <div className="text-sm">
            No surfaces match the current context and filter. Try enabling "Show
            all contexts" or choosing a different surface type.
          </div>
        ),
      });
    }
    ...
    <div className="space-y-4 text-sm">
      ...
      <div className="space-y-1">
        ...
        <div className="text-xs text-neutral-600 dark:text-neutral-400">
          Preset: {editorContext.workspace.activePresetId ?? "Unpinned"}
        </div>
        <div className="text-xs text-neutral-600 dark:text-neutral-400">
                ...
                >
                  ...
                  <div className="text-[11px] text-neutral-500 dark:text-neutral-400 flex flex-wrap gap-2">
                    {surface.supportsContexts &&
                      surface.supportsContexts.length > 0 && (
                        <span>
                          Contexts:{" "}
                          {surface.supportsContexts
                    ...
                    {surface.tags && surface.tags.length > 0 && (
                      <span>
                        Tags: {surface.tags.map((tag) => `#${tag}`).join(" ")}
                      </span>
                    )}
                  </div>
                </article>
