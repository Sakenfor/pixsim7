...
export function GameToolsPanel() {
  ...
  const matchesQuery = (text?: string, tags?: string[]) => {
    ...
    if (tags && tags.some((t) => t.toLowerCase().includes(loweredQuery)))
      return true;
    return false;
  };
  ...
  const orderedSections: CategoryFilter[] = useMemo(() => {
    ...
    const pinned = baseOrder.filter((section) =>
      isSectionPinned(section, mode, primaryView),
    );
    const others = baseOrder.filter(
      (section) => !isSectionPinned(section, mode, primaryView),
    );
              ...
              <section key={section}>
                ...
                <div className="space-y-1">
                  {panels
                    .filter(
                      (p) =>
                        p.category === "workspace" || p.category === "scene",
                    )
              ...
              <section key={section}>
                ...
                {interactions.length === 0 ? (
                  ...
                  <div className="space-y-1">
                    {interactions
                      .filter((plugin) =>
                        matchesQuery(plugin.name, plugin.tags),
                      )
                      .map((plugin) => (
                        <div
                          key={plugin.id}
