    ...
    for (let y = 0; y < composition.layout.rows; y++) {
      for (let x = 0; x < composition.layout.columns; x++) {
        if (!occupied.has(`${x},${y}`)) {
          ...
          for (
            let wx = x;
            wx < x + w && wx < composition.layout.columns;
            wx++
          ) {
    ...
    <div className="h-full flex flex-col bg-white dark:bg-neutral-900">
      ...
      <div className="border-b border-neutral-200 dark:border-neutral-700 p-4">
        ...
        <div className="flex items-center gap-4 text-sm text-neutral-600 dark:text-neutral-400">
          ...
          <span>
            <strong>Grid:</strong> {composition.layout.columns}x
            {composition.layout.rows}
          </span>
          <span>
            <strong>Widgets:</strong> {composition.widgets.length}
          </span>
          {composition.widgets.some(
            (w) => w.dataBindings && Object.keys(w.dataBindings).length > 0,
          ) && (
      ...
      {showDemoPicker && (
        <div className="border-b border-neutral-200 dark:border-neutral-700 p-4 bg-green-50 dark:bg-green-900/10">
          <h3 className="text-sm font-semibold mb-3">
            Demo Compositions with Data Binding
          </h3>
          <p className="text-xs text-neutral-600 dark:text-neutral-400 mb-3">
            These demos showcase the Task 51 data binding system integrated with
            Panel Builder widgets.
          </p>
          <div className="grid grid-cols-3 gap-3">
            ...
            >
              ...
              <div className="text-xs text-neutral-600 dark:text-neutral-400">
                Displays workspace lock status, panel counts, and lists with
                live data from workspace store.
              </div>
            </button>
            <button
            ...
            >
              ...
              <div className="text-xs text-neutral-600 dark:text-neutral-400">
                Shows game mode, world ID, session ID, and full context with
                transforms applied.
              </div>
            </button>
            <button
            ...
            >
              ...
              <div className="text-xs text-neutral-600 dark:text-neutral-400">
                Combines workspace and game state data in a single organized
                panel.
              </div>
            </button>
          </div>
                  ...
                  return (
                    ...
                    >
                      <div className="text-sm font-medium flex items-center gap-2">
                        {widgetDef?.icon}{" "}
                        {widgetDef?.title || widget.widgetType}
                      </div>
