...
export function SessionStateViewer() {
  ...
  const [filter, setFilter] = useState<
    "all" | "arcs" | "quests" | "relationships" | "inventory"
  >("all");
            ...
            <div className="text-xs space-y-1">
              ...
              {world && (
                <div>
                  ...
                  <pre className="inline font-mono text-xs">
                ...
                ([eventId, eventData]: [string, any]) => (
                  <div key={eventId} className="text-xs">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-xs ${eventData.active ? "bg-red-500 text-white" : "bg-neutral-300 dark:bg-neutral-600 text-neutral-700 dark:text-neutral-300"}`}
                      >
  ...
  const renderRelationships = () => {
    ...
    const npcRelationships = entries.filter(
      ([key]) => parseNpcKey(key) !== null,
    );
    const otherRelationships = entries.filter(
      ([key]) => parseNpcKey(key) === null,
    );
                      ...
                      <div className="mt-1 grid grid-cols-2 gap-2">
                        {data.affinity !== undefined && (
                          <div className="flex justify-between">
                            ...
                            <span
                              className={`font-mono ${data.affinity >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                            >
                        ...
                        {data.trust !== undefined && (
                          <div className="flex justify-between">
                            ...
                            <span
                              className={`font-mono ${data.trust >= 0 ? "text-blue-600 dark:text-blue-400" : "text-red-600 dark:text-red-400"}`}
                            >
      ...
      <div className="p-4 space-y-4 overflow-y-auto h-full">
        ...
        {sessions.length === 0 ? (
          <p className="text-sm text-neutral-500">
            No sessions found. Start a game to create a session.
          </p>
        ) : (
          <p className="text-sm text-neutral-500">
            Select a session to view its state.
          </p>
        )}
      </div>
    );
    ...
    <div className="p-4 space-y-4 overflow-y-auto h-full">
      <div className="space-y-2">
        ...
        <p className="text-xs text-neutral-500 dark:text-neutral-400">
          Debug panel for inspecting game session state (flags, relationships,
          arcs, quests)
        </p>
      </div>
      ...
      <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded">
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm font-mono">
            {formatWorldTime(session.world_time)}
          </span>
        </div>
      ...
      {(filter === "all" || filter === "relationships") && (
        <div>
          ...
          {(!session.stats?.relationships ||
            Object.keys(session.stats.relationships).length === 0) && (
            <p className="text-xs text-neutral-500 text-center py-4">
