...
export function TemplateAnalyticsPanel() {
  ...
  // Analytics data
  const allStats = useTemplateAnalyticsStore((state) =>
    state.getAllTemplateStats(),
  );
  const usageRecords = useTemplateAnalyticsStore((state) => state.usageRecords);
  const getTemplateUsage = useTemplateAnalyticsStore(
    (state) => state.getTemplateUsage,
  );
  const getRefactoringHints = useTemplateAnalyticsStore(
    (state) => state.getRefactoringHints,
  );
  const clearAnalytics = useTemplateAnalyticsStore(
    (state) => state.clearAnalytics,
  );
  const clearTemplateAnalytics = useTemplateAnalyticsStore(
    (state) => state.clearTemplateAnalytics,
  );
  ...
  const overviewMetrics = useMemo(() => {
    ...
    const totalNodesInserted = enrichedStats.reduce(
      (sum, stat) => sum + stat.totalNodesInserted,
      0,
    );
    const avgNodesPerInsertion =
      totalInsertions > 0 ? totalNodesInserted / totalInsertions : 0;
    const uniqueScenes = new Set(
      usageRecords.map((r) => r.sceneId).filter(Boolean),
    ).size;
    const uniqueWorlds = new Set(
      usageRecords
        .map((r) => r.worldId)
  ...
  const handleClearTemplate = (templateId: string) => {
    ...
    if (
      confirm(`Clear analytics for "${templateName}"? This cannot be undone.`)
    ) {
  ...
  const renderSeverityBadge = (
    ...
    return (
      <span
        className={`px-2 py-0.5 text-xs font-medium rounded ${colors[severity]}`}
      >
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-sm text-neutral-600 dark:text-neutral-400">
                            ...
                            <span>•</span>
                            <span>
                              {stat.avgNodesInserted.toFixed(1)} avg nodes
                            </span>
                          </div>
                        </div>
            ...
            {sortedStats.length === 0 ? (
              <div className="text-center py-12 text-neutral-500 dark:text-neutral-400">
                ...
                <div className="text-sm mt-1">
                  Insert templates in your scenes to see analytics here
                </div>
              </div>
            ) : (
                      ...
                      <div
                        ...
                        onClick={() =>
                          setSelectedTemplateId(
                            isSelected ? null : stat.templateId,
                          )
                        }
                      >
                      ...
                      {isSelected && selectedTemplateRecords.length > 0 && (
                        <div className="border-t border-neutral-200 dark:border-neutral-700 p-4 bg-neutral-50 dark:bg-neutral-900/50">
                          <h4 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-3">
                            Usage History ({selectedTemplateRecords.length}{" "}
                            records)
                          </h4>
                            ...
                            {selectedTemplateRecords
                              ...
                              .map((record) => (
                                ...
                                >
                                  ...
                                  <div className="text-neutral-600 dark:text-neutral-400">
                                    {record.nodeCount} nodes, {record.edgeCount}{" "}
                                    edges
                                  </div>
                                </div>
                              ))}
          ...
          <div className="space-y-4">
            ...
            {hints.length === 0 ? (
              <div className="text-center py-12 text-neutral-500 dark:text-neutral-400">
                ...
                <div className="text-sm mt-1">
                  Use templates in your scenes to generate insights
                </div>
              </div>
            ) : (
              <div className="space-y-3">
