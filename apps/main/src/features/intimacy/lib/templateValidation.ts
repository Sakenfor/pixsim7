...
export function validateSceneTemplate(template: SceneTemplate): ValidationResult {
  ...
  for (const [idx, gate] of scene.gates.entries()) {
    const gateResult = validateGate(gate);
    if (!gateResult.valid) {
 ...
 */
export function validateArcTemplate(template: ArcTemplate): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  for (const [idx, stage] of arc.stages.entries()) {
    ...
    if (!stage.gate) {
      ...
      const gateResult = validateGate(stage.gate);
      if (!gateResult.valid) {
 ...
 * Validate a gate
 */
function validateGate(gate: RelationshipGate): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
...
function checkProgressionLogic(arc: RelationshipProgressionArc): string[] {
  ...
  // Check if stages progress in a logical order (tier-wise)
  const tierOrder = ['stranger', 'acquaintance', 'friend', 'close_friend', 'lover'];
  ...
  // Check for unreachable stages (gates that can never be satisfied)
  // This is a simplified check - a more sophisticated one would need runtime state
  for (const [idx, stage] of arc.stages.entries()) {
    if (idx === 0) continue; // First stage is always reachable

    const gate = stage.gate;

    // If a stage requires a tier higher than any previous stage provides
 ...
 */
export function validateSceneForTemplate(scene: IntimacySceneConfig): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 */
export function validateArcForTemplate(arc: RelationshipProgressionArc): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
