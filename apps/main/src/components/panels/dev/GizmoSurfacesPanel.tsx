...
export function GizmoSurfacesPanel({ onClose }: GizmoSurfacesPanelProps) {
  const [selectedCategory, setSelectedCategory] = useState<GizmoSurfaceCategory | 'all'>('all');
  const [selectedContext, setSelectedContext] = useState<GizmoSurfaceContext | 'all'>('all');
      ...
      <div className="space-y-2">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {(['scene', 'world', 'npc', 'debug', 'custom'] as const).map(category => (
              <Button
                key={category}
                size="xs"
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {(['scene-editor', 'game-2d', 'game-3d', 'playground', 'workspace'] as GizmoSurfaceContext[]).map(context => (
              <Button
                key={context}
                size="xs"
      ...
      <div className="pt-4 border-t border-neutral-200 dark:border-neutral-700">
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="text-neutral-600 dark:text-neutral-400">
            Total Surfaces: <span className="font-semibold">{allSurfaces.length}</span>
          </div>
          <div className="text-neutral-600 dark:text-neutral-400">
            Shown: <span className="font-semibold">{filteredSurfaces.length}</span>
          </div>
        </div>
      </div>
    </Panel>
        ...
        <div className="p-3 space-y-3 bg-white dark:bg-neutral-900">
          ...
          <div>
            ...
            <div className="flex gap-1">
              {hasPanel && <Badge variant="info" size="sm">Panel</Badge>}
              {hasOverlay && <Badge variant="success" size="sm">Overlay</Badge>}
              {hasHud && <Badge variant="warning" size="sm">HUD</Badge>}
          ...
          {surface.priority !== undefined && (
            <div className="text-xs text-neutral-600 dark:text-neutral-400">
              Priority: <span className="font-semibold">{surface.priority}</span>
            </div>
          )}
            ...
            <div>
              ...
              <div className="text-xs text-neutral-600 dark:text-neutral-400">
                ...
                {surface.requires.permissions && (
                  <div>Permissions: {surface.requires.permissions.join(', ')}</div>
                )}
              </div>
            </div>
          )}
