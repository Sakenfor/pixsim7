 ...
 */
function getCompletions(input: string): { completions: string[]; prefix: string; partial: string } {
  // Find the last expression that starts with 'pixsim'
  const match = input.match(/(pixsim(?:\.[a-zA-Z_][a-zA-Z0-9_]*)*)\.?([a-zA-Z_][a-zA-Z0-9_]*)?$/);
  if (!match) {
...
export function ConsolePanel() {
  ...
  const { history, execute, clear, historyUp, historyDown, resetHistoryNav } = useConsoleStore();
  ...
  const formatEntry = (entry: (typeof history)[0]) => {
    switch (entry.type) {
      ...
      case 'info':
        return <div className="pl-4 text-neutral-500 italic font-mono">{entry.content}</div>;
      default:
        return <div className="pl-4 text-neutral-400 font-mono">{entry.content}</div>;
    }
  };
    ...
    <div className="flex flex-col h-full bg-neutral-900 text-xs">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-neutral-800 bg-neutral-900/80">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-medium text-neutral-300">Console</span>
        </div>
      ...
      >
        {history.length === 0 && (
          <div className="text-neutral-600 italic">
            Type <span className="text-purple-400">pixsim.help()</span> for usage, press{' '}
            <span className="text-purple-400">Tab</span> for autocomplete
          </div>
        )}
        {history.map((entry) => (
