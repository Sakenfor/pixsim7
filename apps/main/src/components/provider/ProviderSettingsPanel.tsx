...
const createInitialFormState = (account: ProviderAccount): EditAccountFormState => ({
  email: account.email,
...
function EditAccountModal({ account, onClose, onSave }: EditAccountModalProps) {
  const [formState, setFormState] = useState<EditAccountFormState>(() => createInitialFormState(account));
  ...
  const handleGoogleFlag = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to connect via Google';
      toast.error(message);
    } finally {
