 ...
 * - NodeTypeRegistry → Available Node Types
 * - NodeRendererRegistry → Custom Renderers
 * - InteractionRegistry → Interaction Plugins
 * - SessionHelperRegistry → Helper Functions
 *
...
function generatePluginDocs(): string {
  ...
  // Header
  sections.push('# 🔌 Plugin Reference\n');
  ...
  sections.push('---\n');
  sections.push('_Generated by `packages/game/engine/src/utils/generatePluginDocs.ts`_\n');
...
function generateNodeTypesSection(): string {
  ...
  allTypes.forEach(type => {
    ...
    const creatable = type.userCreatable ? '✅' : '❌';
  ...
  categoryOrder.forEach(catName => {
    ...
    types.forEach(type => {
      lines.push(`##### ${type.icon || '📦'} ${type.name} (\`${type.id}\`)\n`);
...
function generateNodeRenderersSection(): string {
  ...
  knownRenderers.forEach(renderer => {
    lines.push(`#### \`${renderer.nodeType}\` → ${renderer.component}\n`);
...
function generateInteractionPluginsSection(): string {
  ...
  lines.push('## 🤝 Interaction Plugins\n');
...
function main() {
  ...
  console.log(`📝 Lines: ${docs.split('\n').length}`);
  console.log('');
  console.log('🎉 Done! Check out the generated documentation.');
}
