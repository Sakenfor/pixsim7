...
export function getTierLevel(tier: string): number {
  const index = (TIER_HIERARCHY as readonly string[]).indexOf(tier);
 ...
 */
export const INTIMACY_HIERARCHY = [
  'none',
  'light_flirt',
  'deep_flirt',
  'intimate',
 ...
 */
export function getIntimacyLevel(level: string): number {
  const index = (INTIMACY_HIERARCHY as readonly string[]).indexOf(level);
  return index >= 0 ? index : -1;
}
