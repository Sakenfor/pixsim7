...
export type EditorToGameMessage<TScene = unknown> =
  | LoadSceneMessage<TScene>
  | PlaySceneMessage
  | PauseSceneMessage
  | StopSceneMessage
  | SeekToNodeMessage
 ...
 */
export interface LoadSceneMessage<TScene = unknown> {
  type: "load-scene";
  payload: {
    scene: TScene;
...
export function isEditorToGameMessage<TScene = unknown>(
  msg: any
): msg is EditorToGameMessage<TScene> {
  return (
    msg &&
    typeof msg === "object" &&
...
export function isGameToEditorMessage(msg: any): msg is GameToEditorMessage {
  return (
    ...
    typeof msg === "object" &&
    typeof msg.type === "string" &&
    ["scene-loaded", "scene-error", "playback-state", "node-changed"].includes(
      msg.type
    )
  );
}
