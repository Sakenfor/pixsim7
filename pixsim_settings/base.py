"""
Declarative settings base classes.

Two base classes for two storage scopes:

``SettingsBase`` — global settings (singleton per namespace)::
...
``UserSettingsBase`` — per-user settings (instantiated per request)::
...
class SettingsBase(_UpdateModelMixin, BaseModel, metaclass=_SettingsMeta):
    """
    Base class for global, namespace-scoped settings.

    One singleton per namespace. The in-memory store is populated by each
    consumer's persistence layer (DB appliers, JSON loader, etc.).
    """
    ...
    def _from_cache(cls) -> "SettingsBase":
        ...
        from pixsim_settings.store import get_settings_data
    ...
    def update(self, updates: Dict[str, Any]) -> None:
        ...
        from pixsim_settings.store import apply_settings
...
# ═══════════════════════════════════════════════════════════════════════════
# UserSettingsBase — per-user settings
# ═══════════════════════════════════════════════════════════════════════════

class UserSettingsBase(_UpdateModelMixin, BaseModel, metaclass=_SettingsMeta):
    """
    Base class for per-user settings.

    Not a singleton — instantiate per request from a user's preferences dict.
    """
    ...
    @classmethod
    def for_user(cls, user: Any) -> "UserSettingsBase":
