...
ANALYSIS_SERVICE_ID = "analysis"


class AnalysisGateway:
    def __init__(self, router: ServiceRouter, local_service: AnalysisService) -> None:
        self._router = router
        self.local = local_service

    def has_remote(self) -> bool:
        return self._router.get_client(ANALYSIS_SERVICE_ID) is not None
    ...
    async def request_remote(
        ...
        *,
        ...
        client = self._router.get_client(ANALYSIS_SERVICE_ID)
        if not client:
            return None
        ...
        try:
            ...
            raise ServiceClientError(502, f"Analysis service unavailable: {exc}") from exc
