"""
Social context service - GAME DOMAIN

Builds GenerationSocialContext from game state (world, session, NPC relationships).
This belongs in the GAME service layer, not the generation service layer.

This service is responsible for translating game-specific state into the clean
GenerationSocialContextSchema that the generation service can consume.

DEPENDENCY FLOW:
    Game Domain → Social Context Service → GenerationSocialContextSchema → Generation Service

The generation service has NO knowledge of game models - it only receives data.
"""
import logging
...
async def build_social_context_from_game_state(
    db: AsyncSession,
    world_id: int,
    ...
    """
    Build GenerationSocialContext from game state (world, session, relationships)

    This is a GAME SERVICE function - it knows about GameWorld, GameSession, etc.
    It translates game-specific state into the clean GenerationSocialContextSchema
    that the generation service can consume.

    The generation service should NEVER call this directly - only route handlers
    or other game-layer code should call this.
    ...
    Returns:
        Dict representing GenerationSocialContext with:
            - tier_id: Computed relationship tier ID
            - intimacy_level_id: Computed intimacy level ID
            - relationship_state: Raw relationship values used
            - content_rating: Content rating (clamped by world/user)
            - world_max_rating: World's max rating
            - user_max_rating: User's max rating (if provided)
    ...
    # Build context object matching GenerationSocialContextSchema
    context = {
        'tier_id': tier_id,
        'intimacy_level_id': intimacy_level_id,
        'relationship_state': relationship_values,
        'content_rating': content_rating,
        'world_max_rating': world_max_rating,
    }
    ...
    logger.info(
        f"Built social context from game state: world={world_id}, session={session_id}, npc={npc_id} "
        f"→ tier={tier_id}, intimacy={intimacy_level_id}, rating={content_rating}"
    )
...
def validate_social_context_against_constraints(
    ...
    content_rating = social_context.get('content_rating', 'sfw')
    ...
    intimacy_level_id = social_context.get('intimacy_level_id', '')
    intimacy_band = _map_intimacy_to_band(intimacy_level_id)
    if intimacy_band in ['deep', 'intense']:
        warnings.append(
            f"High intimacy level '{intimacy_level_id}' may produce intense content"
        )
