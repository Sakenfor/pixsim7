...
class ParserHintProvider:
    ...
    async def get_active_packs(
        ...
        if pack_ids:
            ...
            # Load all packs with the given status
    ...
    def build_role_keyword_map(
        ...
        Multiple packs can contribute keywords to the same role/attribute.
        ...
        for pack in packs:
            for key, words in pack.parser_hints.items():
                # Initialize list for this key if not present
                if key not in merged:
                    merged[key] = []

                # Add words, avoiding duplicates
                for word in words:
    ...
    def build_keyword_map_sync(
        ...
        Useful when packs are already loaded and no async DB access is needed.
        """
    ...
    def extract_role_hints(
        ...
        """
        # Support both prefixed and unprefixed role names
    ...
    async def get_hints_for_packs(
        ...
        Returns:
            Merged hint map ready for parser use
        """
...
async def build_role_keyword_map_from_db(
    ...
    This is the function signature suggested in the task spec.
    """
