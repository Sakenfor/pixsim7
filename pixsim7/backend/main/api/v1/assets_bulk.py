...
async def bulk_get_assets(
    ...
    """
    # Ownership-scoped fetch is inherently per-id, but the response builder is
    # NOT: collect the assets, then build them in one shot so tags + has_children
    # batch into a single query each (instead of 2 queries PER asset). This is
    # the path the burst-time WS/thumbnail refresh coordinator funnels into, so
    # the N→1 query collapse matters under load.
    assets: List = []
    for asset_id in request.asset_ids[:_BULK_GET_CAP]:
        try:
            ...
            continue
    return await build_asset_responses_with_tags(assets, db)
