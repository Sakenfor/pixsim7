...
async def get_generation_config(user: CurrentUser):
    ...
    return GenerationConfigResponse(
        ...
        auto_retry_enabled=gs.auto_retry_enabled,
...
async def update_generation_config(
    ...
    logger.info(
        ...
        gs.auto_retry_enabled,
        gs.auto_retry_max_attempts,
        gs.max_jobs_per_user,
        gs.max_accounts_per_user,
    )
...
from pixsim7.backend.main.services.generation.worker_settings import (
    GenerationWorkerSettings,
    get_worker_settings,
)

GenerationWorkerConfigUpdate = GenerationWorkerSettings.get_update_model()


@router.get("/admin/generation-worker/config", response_model=GenerationWorkerSettings)
...
@router.patch("/admin/generation-worker/config", response_model=GenerationWorkerSettings)
...
class FilteredRetryOverrideUpdate(BaseModel):
    """Per-operation filtered-retry override. Omit/null a field to leave it unset
    (cap omitted → backoff-only); send an empty body to clear the operation."""
    cap: int | None = Field(default=None, ge=1, le=100)
    defer_seconds: int | None = Field(default=None, ge=1, le=600)


@router.patch(
    "/admin/generation-worker/filtered-retry/{operation_type}",
    response_model=GenerationWorkerSettings,
)
async def update_filtered_retry_override(
    operation_type: str,
    ...
    entry = {
        k: v
        for k, v in {"cap": body.cap, "defer_seconds": body.defer_seconds}.items()
        if v is not None
    }
    if entry:
...
LLMConfigUpdate = LLMSettings.get_update_model()


@router.get("/admin/llm/config", response_model=LLMSettings)
async def get_llm_config(user: CurrentUser):
    """Get current LLM cache config (any authenticated user)."""
    return get_llm_settings()


@router.patch("/admin/llm/config", response_model=LLMSettings)
async def update_llm_config(
    ...
    logger.info(
        ...
        admin.username,
        ls.llm_cache_enabled,
        ls.llm_cache_ttl,
        ls.llm_cache_freshness,
    )
