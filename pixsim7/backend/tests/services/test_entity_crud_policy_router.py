...
@pytest.mark.asyncio
async def test_primary_create_route_propagates_policy_violation_shape(monkeypatch) -> None:
    router = APIRouter()
    spec = _spec()
    crud_router._register_create_route(router, spec)
    ...
    def _reject(**kwargs):
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "contract": "/api/v1/game/meta/authoring-contract",
            },
        )

    monkeypatch.setattr(crud_router, "_enforce_domain_policy_or_400", _reject)
    monkeypatch.setattr(crud_router, "TemplateCRUDService", _FakeTemplateService)

    endpoint = _route_endpoint(router, "/widgets", "POST")
    with pytest.raises(HTTPException) as exc:
        await endpoint(
            data={"name": ""},
            world_id=None,
            session_id=None,
            db=object(),
            current_user=SimpleNamespace(id=1, principal_type="agent"),
        )
...
@pytest.mark.asyncio
async def test_nested_replace_all_route_propagates_policy_violation_shape(monkeypatch) -> None:
    router = APIRouter()
    spec = _spec()
    nested = _nested()
    crud_router._register_nested_entity_routes(router, spec, nested)
    ...
    endpoint = _route_endpoint(router, "/widgets/{parent_id}/hotspots", "PUT")
    parent_id = str(UUID("00000000-0000-0000-0000-000000000099"))
    with pytest.raises(HTTPException) as exc:
        await endpoint(
            parent_id=parent_id,
            data={"items": []},
            ...
            session_id=None,
            db=object(),
            current_user=SimpleNamespace(id=2, principal_type="agent"),
        )
