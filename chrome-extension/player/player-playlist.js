...
(function() {
  ...
  async function addToPlaylist(name, url, isLocal, isImage, source) {
  ...
  function selectPlaylistItem(itemId) {
    ...
    if (item.isLocal) {
      // Offer to re-open the file
      showToast('Opening file picker - select the same file', true);
  ...
  function renderPlaylist() {
    ...
    elements.playlistItems.innerHTML = items.map(item => {
      ...
      const localBadge = item.isLocal ? '<span style="opacity:0.5">(local)</span>' : '';
